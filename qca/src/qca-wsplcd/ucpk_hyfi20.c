...
void ucpkHyfi20GetWPAPsk(unsigned char key[32], char psk[63])
{
    ...
    for (i=0; i<31; i++ )
    {
       snprintf((char *)&psk[i*2], 63 - i*2, "%02x", digest[i]);
    }
...
void ucpkHify20Get1901NMK(unsigned char key[32], char nmk[33])
{
    ...
    for (i=0; i<16; i++ )
    {
       snprintf((char *)&nmk[i*2], 33 - i*2, "%02x", digest[i]);
    }
}
...
void ucpkTest()
{
    ...
    for (i =0; i< 32; i++)
        snprintf(&PSKHex[i*2], sizeof(PSKHex) - i*2, "%02x", PSK[i]);
    ...
    for (i =0; i< 32; i++){
        snprintf(&keyHex[i*2], sizeof(keyHex) - i*2, "%02x", key[i]);
    }
