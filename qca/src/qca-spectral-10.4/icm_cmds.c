                                                 ...
                                                 ICM_MSG_T* pmsg)
{
    ...
    if (i == pdev->conf.num_radios) /* We didn't find a matching picm */
    {
                                              ...
                                              ICM_MSG_T* pmsg)
{
    ...
    if (strlcpy(temp_dev_ifname, picm->dev_ifname, IFNAMSIZ) >= IFNAMSIZ) {
        fprintf(stderr, "dev_ifname too long: %s\n", picm->dev_ifname);
        goto bad;
    }
    /* XXX - Can restore after scan. But
       for now, it won't be required */
    if (strlcpy(picm->dev_ifname, preq->deviceName, IFNAMSIZ) >= IFNAMSIZ) {
        fprintf(stderr, "deviceName too long: %s\n", preq->deviceName);
        goto bad;
    }
    ...
    if (icm_activate_scanner_thread(picm) == FAILURE) {
        ...
        rsp_status = ICM_RSP_STATUS_FAILURE;
        if (strlcpy(picm->dev_ifname, temp_dev_ifname, IFNAMSIZ) >= IFNAMSIZ) {
            fprintf(stderr, "temp_dev_ifname too long: %s\n", temp_dev_ifname);
            goto bad;
        }
        goto bad;
    }
...
static int icm_handle_channel_set_req(ICM_DEV_INFO_T* pdev, ICM_MSG_T* pmsg)
{
    ...
    memset(chanset, 0, sizeof(ICM_CHANSET_JOB_T));

    /* TBD: Re-run best channel selection with latest channel config,
       exclusions etc */
    if (strlcpy(chanset->dev_ifname, preq->deviceName, IFNAMSIZ) >= IFNAMSIZ) {
        fprintf(stderr, "deviceName too long: %s\n", preq->deviceName);
        goto out;
    }
    if (strlcpy(picm->dev_ifname, preq->deviceName, IFNAMSIZ) >= IFNAMSIZ) {
