...
ath_nodekick_event_defer(void *data){
    ...
    TAILQ_FOREACH_SAFE(ni, &nt->nt_node, ni_list, next) {
        ...
        if( ( (u_int32_t)1 << nodeindex) & scn->sc_htc_nodekickmask ){
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "nodekick %d \n",nodeindex);
    ...
    if (nodeindex == 0xff) {
        ...
        if (vap->iv_create_flags & IEEE80211_P2P_DEVICE) {
            ...
            if (nodeindex == 0xff) {
                ...
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s new found nodeindex(%d) for p2p\n", __FUNCTION__, nodeindex);
            }
        } else
      #endif
        nodeindex = 0;
...
ath_action_tx_event(void *Context, int8_t tx_status)
{
    ...
    if (scn->sc_p2p_action_queue_head) {
        ...
        if (!p2p_action_wbuf->deleted) {
            ...
            QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "### action frame %p marked as deleted\n", wbuf);
#endif        
        }
