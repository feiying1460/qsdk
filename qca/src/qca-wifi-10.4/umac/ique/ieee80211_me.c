    ...
    while(curr_grp_list != NULL){
        ...
        if(!TAILQ_FIRST(&curr_grp_list->src_list)){
...
#ifdef QCA_NSS_WIFI_OFFLOAD_SUPPORT
            {
                ...
                struct ieee80211com *ic = vap->iv_ic;
                if (osifp->nss_wifiol_ctx && ic->nss_funcs) {
                    ic->nss_funcs->ic_osif_nss_vdev_me_delete_grp_list(vap->iv_txrx_handle, curr_grp_list->group_addr, (u_int8_t *)&curr_grp_list->u, (IEEE80211_IS_MULTICAST(curr_grp_list->group_addr) ? ETHERTYPE_IP : ETHERTYPE_IPV6));
                }
            }
#endif
            OS_FREE(curr_grp_list);
