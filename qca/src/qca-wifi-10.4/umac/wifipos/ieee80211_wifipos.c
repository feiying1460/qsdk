...
QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: %d: hc(%d) oc(%d)\n", __FUNCTION__, __LINE__,
	...
	if (ni == NULL) {
        ...
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: %d: create tmp node(%p)\n", __FUNCTION__, __LINE__, ni);
	}
...
int32_t ieee80211com_init_netlink(struct ieee80211com *ic)
{
    ...
    while ((scn_num < ol_num_global_scn) && (scn_idx < GLOBAL_SCN_SIZE)) {
        ...
        if (ic_tmp->rtt_sock != NULL) {
            ...
            QDF_PRINT_INFO(ic_tmp->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Socket already created %p\n", __func__, ic->rtt_sock);
            return 0;
        }
    }
        ...
        if (ic->rtt_sock == NULL) {
            ...
            if (ic->rtt_sock == NULL) {
                ...
                QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: %d: Wifipos 2nd Netlink socket created:%p\n",
                    __FUNCTION__, __LINE__, ic->rtt_sock);
        } else{
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, KERN_INFO"%s: %d: Wifipos 1st Netlink socket created:%p\n",
                    __FUNCTION__, __LINE__, ic->rtt_sock);
        }
...
int ieee80211_wifipos_vattach(struct ieee80211vap *vap)
{
    ...
    if (vap->iv_wifipos == NULL) {
        ...
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_WIFIPOS, "BRINGUP:pointer of pvt_wifiposdata is %p and socket val is %p and socket id is %d\n", pvt_wifiposdata, pvt_wifiposdata->wifipos_sock, pvt_wifiposdata->process_pid);
		pvt_wifiposdata->state = IEEE80211_WIFIPOSMGR_STATE_IDLE;
	...
	IEEE80211_DPRINTF(vap, IEEE80211_MSG_WIFIPOS, "%s: %d: Wifipos intialization complete with ic:%p\n", __FUNCTION__, __LINE__,ic);
