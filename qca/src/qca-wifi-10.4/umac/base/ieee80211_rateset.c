...
ieee80211_derive_nss_from_cap(struct ieee80211_node *ni)
{
    ...
    u_int32_t nss_vht160 = 0;
    u_int32_t nss_vht80_80 = 0;
    bool status = true;
 ...
 */
	...
	case IEEE80211_EXTNSS_MAP_23_80F2_160F1_80P80F1:
            ni->ni_rxstreams *= 2;
            nss_vht160 = (ni->ni_rxstreams / 2);
            nss_vht80_80 = (ni->ni_rxstreams / 2);
            break;

        case IEEE80211_EXTNSS_MAP_13_80F2_160F2_80P80F1:
            ni->ni_rxstreams *= 2;
            nss_vht160 = (ni->ni_rxstreams);
            nss_vht80_80 = (ni->ni_rxstreams / 2);
            break;

        case IEEE80211_EXTNSS_MAP_03_80F1_160FDOT75_80P80FDOT75:
            nss_vht160 = ((ni->ni_rxstreams * 3) / 4);
            nss_vht80_80 = ((ni->ni_rxstreams * 3) / 4);
            break;
    ...
    if (nss_vht160) {
        ni->ni_bwnss_map = IEEE80211_BW_NSS_FWCONF_160(nss_vht160 - 1);
    }

    if (nss_vht80_80) {
        ni->ni_bwnss_map |=  IEEE80211_BW_NSS_FWCONF_80_80(nss_vht80_80 - 1);
    }
    return status;
}
    ...
    if (ni->ni_chwidth == IEEE80211_CWM_WIDTH160) {
        ...
        if (!(ni->ni_bwnss_map & IEEE80211_BW_NSS_FWCONF_MAP_ENABLE)) {
            ...
            if (!ieee80211_get_bw_nss_mapping(vap, &nssmap, rx_chainmask) &&
                (tx_rrs.num_streams < (nssmap.bw_nss_160 + 1) * 2)) {
                tx_rrs.num_streams = nssmap.bw_nss_160 + 1;
            } else {
            ...
            if (vap->iv_ext_nss_capable && ni->ni_ext_nss_support) {
                ...
                if (!ieee80211_get_bw_nss_mapping(vap, &nssmap, tx_chainmask)) {
                    u_int32_t rx_nss_160 = 0;
                    u_int32_t rx_nss_80_80 = 0;
                    rx_nss_160 = QDF_MIN(nssmap.bw_nss_160, IEEE80211_GET_BW_NSS_FWCONF_160(ni->ni_bwnss_map));
                    rx_nss_80_80 = QDF_MIN(nssmap.bw_nss_160, IEEE80211_GET_BW_NSS_FWCONF_80_80(ni->ni_bwnss_map));
                    ni->ni_bwnss_map = IEEE80211_BW_NSS_FWCONF_160(rx_nss_160) | IEEE80211_BW_NSS_FWCONF_80_80(rx_nss_80_80);
                }
            } else {
                u_int8_t tx_chainmask = ieee80211com_get_tx_chainmask(ic);
                if (!ieee80211_get_bw_nss_mapping(vap, &nssmap, tx_chainmask)) {
                    u_int32_t min_bwnss_map = QDF_MIN(nssmap.bw_nss_160, IEEE80211_GET_BW_NSS_FWCONF_160(ni->ni_bwnss_map));
                    ni->ni_bwnss_map = IEEE80211_BW_NSS_FWCONF_160(min_bwnss_map);
                } else {
                    ni->ni_bwnss_map = 0;
                }
           }
 ...
 */
int
ieee80211_check_rate(struct ieee80211vap *vap, struct ieee80211_channel *chan, struct ieee80211_rateset *rrs, int is_ht)
{
#define	RV(v)	((v) & IEEE80211_RATE_VAL)
    struct ieee80211com *ic = vap->iv_ic;
    struct ieee80211_rateset *srs;
