...
print_target_node_bitmap(struct ieee80211_node *ni)
{
    ...
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : ", __FUNCTION__);
    for (i = 0; i < HTC_MAX_NODE_NUM; i++) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%d ", ni->ni_ic->target_node_bitmap[i].ni_valid);
    }    
...
{
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "------------------------------------------------------------\n");
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ni_associd: 0x%x\n", nt->ni_associd);
    //printk("ni_txpower: 0x%x\n", nt->ni_txpower);

    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "mac: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
        nt->ni_macaddr[0], nt->ni_macaddr[1], nt->ni_macaddr[2],
        nt->ni_macaddr[3], nt->ni_macaddr[4], nt->ni_macaddr[5]);
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "bssid: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
        nt->ni_bssid[0], nt->ni_bssid[1], nt->ni_bssid[2],
        nt->ni_bssid[3], nt->ni_bssid[4], nt->ni_bssid[5]);     

    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "nodeindex: %d\n", nt->ni_nodeindex);
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "vapindex: %d\n", nt->ni_vapindex);
    //printk("ni_vapnode: 0x%x\n", nt->ni_vapnode);
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ni_flags: 0x%x\n", nt->ni_flags);
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ni_htcap: %d\n", nt->ni_htcap);
    //printk("ni_valid: %d\n", nt->ni_valid);
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ni_capinfo: 0x%x\n", nt->ni_capinfo);
    //printk("ni_ic: 0x%08x\n", nt->ni_ic);
    //printk("ni_vap: 0x%08x\n", nt->ni_vap);
    //printk("ni_txseqmgmt: %d\n", nt->ni_txseqmgmt);
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ni_is_vapnode: %d\n", nt->ni_is_vapnode);
    QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "ni_maxampdu: %d\n", nt->ni_maxampdu);
    //printk("ni_iv16: %d\n", nt->ni_iv16);   
    //printk("ni_iv32: %d\n", nt->ni_iv32);   
#ifdef ENCAP_OFFLOAD
...
ieee80211_chk_vap_target(struct ieee80211com *ic)
{
	...
	for (i = 0; i < HTC_MAX_VAP_NUM; i++) {
		...
		if (ic->target_vap_bitmap[i].vap_valid == 0) {
			QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: target_vap_bitmap(%d) is invalid\n", __FUNCTION__, i);
			break;
		}
	}
	...
	if (i == HTC_MAX_VAP_NUM) {
		QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: exceed maximum vap number\n", __FUNCTION__);
		return 1 ;
	}
...
ieee80211_add_vap_target(struct ieee80211vap *vap)
{
...
#if 0
    /* Dump mac address */
    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s, mac: 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x\n",
    ...
    for (i = 0; i < HTC_MAX_VAP_NUM; i++) {
        ...
        if (IEEE80211_ADDR_EQ(ic->target_vap_bitmap[i].vap_macaddr, vap->iv_myaddr)) {
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: add the same vap twice\n", __FUNCTION__);
            return;
        }
...
#if 0        	
            QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: target_vap_bitmap(%d) vap_valid = %d\n", __FUNCTION__, i,
            ic->target_vap_bitmap[i].vap_valid);
#endif            
        }
    }

    QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: vap index: %d\n", __FUNCTION__, vapindex);
...
ieee80211_chk_node_target(struct ieee80211com *ic)
{
   int i;
	...
	if (i == HTC_MAX_NODE_NUM) {
		QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: exceed maximum node number\n", __FUNCTION__);
		return 1;
	}
	return 0;
    ...
    u_int8_t is_vap_node)
{
    ...
    for (i = 0; i < HTC_MAX_NODE_NUM; i++) {
        ...
        if (IEEE80211_ADDR_EQ(ni->ni_ic->target_node_bitmap[i].ni_macaddr, ni->ni_macaddr)) {
            QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: add the same node : Updating Target node  %d\n", __FUNCTION__, i);
    ...
    if (i == HTC_MAX_NODE_NUM) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: exceed maximum node number\n", __FUNCTION__);
        return;
    }
...
ieee80211_delete_vap_target(struct ieee80211vap *vap)
{
    ...
    if (i == HTC_MAX_VAP_NUM) {
        QDF_PRINT_INFO(ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Can't find desired vap\n", __FUNCTION__);
        return;
    }
...
IEEE80211_NODE_LEAVE(struct ieee80211_node *ni)
{
  ...
  if(OS_IN_SOFTIRQ()){
      _ieee80211_node_leave_defer(ni)  ;
    return 0;
  }
  else
