...
u_int16_t ieee80211_SNRToPhyRateTablePerformLookup(
    ...
    if ((vap->iv_cur_mode == IEEE80211_MODE_11AC_VHT160) || (vap->iv_cur_mode == IEEE80211_MODE_11AC_VHT80_80))
    {
        if ((scn->target_type == TARGET_TYPE_QCA9888) && (nss>1)) {
            /* BESRA in VHT60/VHT80_80 mode is capable of max rate of nss 1 */
            nss=1;
        }
        else if((scn->target_type == TARGET_TYPE_QCA9984) && (nss>2) ) {
            /* CASACDE in VHT160/VHT80_80 mode is capable of max rate of nss 2 */
            nss=2;
        }
    }
    ...
    if (0 == i) {
        ...
        IEEE80211_DPRINTF(vap, IEEE80211_MSG_IOCTL,
                "Rate table: PhyMode-idx %d ChWidth-idx %d Stream-idx %d SNR-idx %d\n",
                phyMode, chwidth, (nss-1), (i-1));
