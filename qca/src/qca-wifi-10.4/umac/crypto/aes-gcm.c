...
void * aes_encrypt_init(const u8 *key, size_t len)
{
        u32 *rk;
        int res;
        rk = (u32 *)os_malloc(NULL, AES_PRIV_SIZE, GFP_ATOMIC);
        if (rk == NULL)
            return NULL;
...
void aes_encrypt_deinit(void *key)
{
    memset(key, 0, AES_PRIV_SIZE);
    OS_FREE(key);
}
	       ...
	       const u8 *aad, size_t aad_len, u8 *crypt, u8 *tag)
{
	...
	if (aes == NULL){
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "aes_gcm :: %s[%d]",__func__,__LINE__);
		return -1;
        }
