...
ol_if_dfs_attach(struct ieee80211com *ic, void *ptr, void *radar_info)
{
	...
	qdf_print(KERN_INFO"%s: called; ptr=%p, radar_info=%p\n",
	    __func__,
	    ptr,
	    radar_info);
    ...
    struct dfs_nol_chan_entry *nollist, int nentries)
{
	...
	ieee80211_enumerate_channels(ichan, ic, i) {
		...
		if (nol_found && (! IEEE80211_IS_CHAN_RADAR(ichan)))
			qdf_print("%s: radar found = ichan=%p, freq=%d, "
			...
			qdf_print("%s: NOL clear = ichan=%p, freq=%d, "
			    "vht freq1=%d, flags=0x%x\n",
			    __func__,
			    ichan,
			    ichan->ic_freq,
	...
	switch (dfsdomain) {
		case DFS_FCC_DOMAIN:
			...
			rinfo.dfsdomain = DFS_FCC_DOMAIN;
                        /* china uses a radar pattern that is similar to ETSI
                           but it follows FCC in all other respect like
                           transmit power, CCA threshold etc.
                        */
                        if (ic->ic_country.countryCode == CTRY_CHINA) {
			    qdf_print("%s: FCC domain -- Country China(156) override FCC radar pattern\n", __func__);
		...
		case DFS_ETSI_DOMAIN:
			...
			rinfo.numradars = ARRAY_LENGTH(dfs_etsi_radars);
			    ...
			    qdf_print("%s: ETSI domain -- Korea(412)\n", __func__);
