    ...
    qdf_nbuf_t tail_msdu)
{
    ...
    if(peer->tids_rx_reorder[tid].array == NULL) {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:tids_rx_reorder.array for tid %d is NULL for peer %p,"
    ...
    u_int8_t tid)
{
    ...
    if (rx_reorder->array != &rx_reorder->base){
        ...
        qdf_print("Check BA session state.Peer:%p,id:%d,tid:%d\n",peer,peer_id,tid);
