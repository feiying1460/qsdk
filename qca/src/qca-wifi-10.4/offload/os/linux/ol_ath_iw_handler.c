...
#endif
    { OL_ATH_PARAM_TXPOW_MGMT | OL_ATH_PARAM_SHIFT,
        IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 2, 0, "set_txpow_mgmt" },
    { OL_ATH_PARAM_TXPOW_MGMT | OL_ATH_PARAM_SHIFT, IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1,
        IW_PRIV_TYPE_INT | IW_PRIV_SIZE_FIXED | 1, "get_txpow_mgmt" },
                           ...
                           char *extra)
{
    ...
    if (param == (OL_ATH_PARAM_TXPOW_MGMT | OL_ATH_PARAM_SHIFT)) {
        ...
        if( !(ol_txpow_mgmt_frm_valid(i[1],i[2],&value)) ) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "[%s]: Invalid arguments provided\n",__func__);
            return -EINVAL;
        }
    }
...
static int ol_ath_iw_getparam(struct net_device *dev, struct iw_request_info *info, void *w, char *extra)
{
    ...
    if (val[0] == (OL_ATH_PARAM_TXPOW_MGMT | OL_ATH_PARAM_SHIFT)) {

        if ( ((val[1] & ~IEEE80211_FC0_SUBTYPE_MASK)!=0) || (val[1] < IEEE80211_FC0_SUBTYPE_ASSOC_REQ)
            || (val[1] > IEEE80211_FC0_SUBTYPE_DEAUTH) ) {
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Invalid value entered for frame subtype\n");
            return -EINVAL;
        }
        return ol_ath_ucfg_getparam(scn, val[0], val);
    }
