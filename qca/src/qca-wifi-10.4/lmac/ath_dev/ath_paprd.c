...
int ath_paprd_cal(struct ath_softc *sc)
{
    ...
    {
        /* Intentional print */
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s sc %p chan_mask %d PAPRD excessive failure disabling PAPRD now\n", __func__, sc, sc->sc_tx_chainmask);
...
int ath_tx_paprd_init(struct ath_softc *sc)
{
...
#if 0
    {
        ...
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s sc %p ---Enable PAPRD DEBUG ---- fail count %d\n", __func__, sc, sc->sc_paprd_failed_count);
    }
#endif
    ...
    if ( ath_hal_getcapability(sc->sc_ah, HAL_CAP_PAPRD_ENABLED, 0, &result) != HAL_OK) {
        DPRINTF(sc, ATH_DEBUG_UNMASKABLE, "%s sc %p PAPRD disabled in HAL\n",
                __func__, sc);
        return 0;
    }
    DPRINTF(sc, ATH_DEBUG_UNMASKABLE, "%s sc %p PAPRD Enabled\n",
            __func__, sc);
