        ...
        struct ieee80211_rateset *negotiated_htrates)
{
    ...
    if (node_index >= ATH_HTC_MAX_NODE_NUM) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: node_index exceeds the HTC capability, node_index = %d !\n", __func__, node_index);
        return;
    }
...
ath_htc_rx(ath_dev_t dev, wbuf_t wbuf)
{
    ...
    if (bf == NULL)
    {
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: bf is NULL\n", __func__);
        wbuf_free(wbuf);
        ATH_RXBUF_UNLOCK(sc);
        return;
    }
...
ath_htc_rx_tasklet(ath_dev_t dev, int flush)
{
    ...
    do {
        ...
        if (flush == RX_DROP) {
             ...
             */
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: drop rx frame when flush rx queue\n", __func__);
...
#if 0    
    if(rxhdr->numseg > 1){
        QDF_PRINT_INFO(sc ->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " isamsdu %d rxhdr->numseg %d  rxhdr->sgseqno %d  seq %d leb %d \n",rxhdr->is_seg,rxhdr->numseg,rxhdr->sgseqno,rxhdr->res,wbuf_get_pktlen(wbuf));
...
ath_htc_uapsd_sendqosnull(struct ath_softc *sc, struct ath_node *an, u_int8_t ac)
{
    ...
    if (wbuf == NULL) {
        ...
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: qosnul buffers unavailable \n",__func__);
        return;
    }
...
ath_htc_process_uapsd_trigger(ath_dev_t dev, ath_node_t node, u_int8_t maxsp, u_int8_t ac, u_int8_t flush, bool *sent_eosp, u_int8_t maxqdepth)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if(ath_htc_send_pkt(dev, wbuf, sc->sc_uapsd_ep, an) != 0) {
            ...
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, " %s unable to send UAPSD pkts,SW queue depth %d\n",__FUNCTION__,an->an_uapsd_qdepth);
            ni->ni_flags |= IEEE80211_NODE_UAPSD_CREDIT_UPDATE;
            goto uapsd_q;
        }
#else        
...
//                    wbuf_free(wbuf1);
//#endif                
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Rejecting unicast encrypted frame as key is not set\n");
                return 0;
            }
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "clearing privacy bit for mcast packet\n");
...
queue:
    ...
    else
    {
        ...
        if (qdf_unlikely((htcstatus != A_OK) )){
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Freeing Management pkt due to credit unavailibility \n");
            wbuf_free(wbuf);
            wbuf = NULL;
        }
#endif
...
#define ATH_GMAC_FRAG_BUF_SIZE          512
    ...
    if (tid == NULL) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "wrong tid, tidno %d\n",tfh->tidno);
        return -1;
    }
...
ath_htc_rx_init(ath_dev_t dev, int nbufs)
{
    ...
    do {
        ...
        if (error != 0) {
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "failed to allocate rx descriptors: %d\n", error);
            break;
        }
...
ath_wmi_get_target_stats(ath_dev_t dev, HTC_HOST_TGT_MIB_STATS *stats )
{
    ...
    if(cmd_status) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s, get_status fail = %x\n", __FUNCTION__, cmd_status);
    }
    else {
...
ath_htc_beacon_generate(struct ath_softc *sc, int if_id, int *needmark)
{
     ...
     */
    {
        ...
        if (wbuf_org != NULL) {
            ...
            if (wbuf == NULL) {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s : allocate wbuf fail for beacon\n", __func__);
                return NULL;
            }
            else {
...
ath_wmi_beacon_stuck_internal(struct ath_softc *sc)
{
    ...
    //printk("bmiss %d \n", sc->sc_bmisscount);  
#else
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: bmisscount = %d, show_cycles = %d, rx_clear = %d, rx_frame = %d, tx_frame = %d\n", __func__, sc->sc_bmisscount, show_cycles, rx_clear, rx_frame, tx_frame);
#endif    
#ifdef ATH_SUPPORT_DFS    
    ...
    if((sc->sc_3stream_sigflag1) && (sc->sc_3stream_sigflag2)) {
            ...
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s engaing fix 0x9944 %x 0x9970 %x\n",__FUNCTION__,OS_REG_READ(sc->sc_ah,0x9944),
    ...
    if (needmark) {
        ...
        if (((AH_TRUE == sc->sc_hang_check) && ath_hw_hang_check(sc)) ||
            ...
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: --- Beacon Stuck ---\n", __func__);
...
//        tsf = ath_hal_gettsf64(ah);
    ...
    if (bfaddr != 0)
    {
        ...
        if (wbuf_hdrspace(wbuf) < need_headroom)
        {
            ...
            if (wbuf == NULL) {
                QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Insufficient header room space and allocate new buffer fail I\n", __func__);
                return;
            }
        }
#endif
        ...
        if (qdf_unlikely((status != A_OK) )){
            QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Freeing Beacon pkt due to credit unavailibility \n");
			wbuf_free(wbuf);
            wbuf = NULL;
...
/* Dump Data content */
	{
		...
		{
			QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%02x ", p[ii]);
...
#ifdef  ATH_HTC_TX_SCHED
    if (qdf_unlikely((status != A_OK) )){
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Freeing adhoc beacon pkt due to credit unavailibility \n");
        wbuf_free(wbuf);
        wbuf = NULL;
...
void ath_htc_host_txep_init(struct ath_softc *sc)
{
    ...
    QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: Epnum to Hardware Queue Mapping\n", __func__);
    ...
    for (ep = 0; ep < NUM_TX_EP; ep++)
    {
        ...
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s:epnum=%d hqnum=%d\n", __func__, txep->ep_num,txep->hwq_num);
    }
...
void ath_htc_uapsd_credit_update(ath_dev_t dev, ath_node_t node)
{
    ...
    ATH_HTC_UAPSD_LOCK(sc);
    if (an == NULL) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s an is NULL \n",__FUNCTION__);
        ATH_HTC_UAPSD_UNLOCK(sc);
        return;
    }    
...
ath_wmi_set_btcoex(struct ath_softc *sc, u_int8_t dutyCycle, u_int16_t period,  u_int8_t stompType)
{
    ...
    if(cmd_status) {
        QDF_PRINT_INFO(sc->sc_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "Host : WMI COMMAND READ FAILURE stat = %x\n", cmd_status);
