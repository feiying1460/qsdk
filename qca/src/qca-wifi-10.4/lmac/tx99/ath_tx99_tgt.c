...
ath_tgt_tx99(struct ath_softc *sc, int chtype)
{
    ...
    for (i = 0; i < totalbuffer; i++)
    {
        ...
        if(bf ==  NULL)
        {
            qdf_print("ath_tgt_tx99: allocate ath buffer fail:%d\n",i+1);
            return -EINVAL;
        }
    	...
    	if (r == AH_FALSE) {
            qdf_print("%s: fail fill tx desc r(%d)\n", __func__, r);
            return -EINVAL;
    	}
    ...
    TAILQ_FOREACH(tbf, &sc->tx99_tmpq, bf_list) {
        tx99_bus_dma_sync_single(sc->sc_osdev, tbf->bf_buf_addr[0], (buf->end-buf->data),
    ...
    ah_tx99_start(sc->sc_ah, (uint8_t *)txq->axq_qnum);
...
int ath_tx99_tgt_start(struct ath_softc *sc, int chtype)
{
    ...
    qdf_print("testmode:%d\n",sc->sc_tx99->testmode);
    qdf_print("txrate:%d\n",sc->sc_tx99->txrate);
    qdf_print("txpower:%d\n",sc->sc_tx99->txpower);
    qdf_print("txchain:%d\n",sc->sc_tx99->chanmask);
    qdf_print("htmode:%d\n",sc->sc_tx99->htmode);
    qdf_print("type:%d\n",sc->sc_tx99->type);
    qdf_print("channel type:%d\n",chtype);
