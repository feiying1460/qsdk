        ...
        if (ret == 0) {
            ...
            if (pl_dev && (((pl_dev->pl_info->log_state) & ATH_PKTLOG_TX_CAPTURE_ENABLE) == 0)) {
                QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "pl_dev:%p; pl_funcs:%p; pktlog_enable:%p; scn:%p\n",
                                        pl_dev, pl_dev->pl_funcs,
                                        pl_dev->pl_funcs->pktlog_enable,
                                        pl_dev->scn);
...
pktlog_read(struct file *file, char *buf, size_t nbytes, loff_t *ppos)
{
    ...
    if (!pktlog_hdr_size) {
        ...
        QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: WARNING: pl_info->pktlog_hdr_size = %u, Using pktlog_hdr_size as: %u\n",
