...
#define ATH_SET_VAP_BSSID_MASK_ALTER(bssid_mask) \
      do {        \
           ((bssid_mask)[0] &= ~(((ATH_VAP_ID_MASK_ALTER) >> 4) | 0x02)); \
...
#define ATH_SET_VAP_BSSID_MASK_ALTER(bssid_mask)      ((bssid_mask)[0] &= ~(((ATH_VAP_ID_MASK_ALTER) << 4) | 0x02))
    ...
    do {                                                 \
        if (ic->ic_is_macreq_enabled(ic)) { \
            ...
            temp_bssid = ( ( bssid1 | bssid2   ) & ATH_VAP_ID_MASK ) \
                                   >> ( OCTET - width ) ;    \
            if ( bssid[0] & IEEE802_MAC_LOCAL_ADMBIT )  {      \
