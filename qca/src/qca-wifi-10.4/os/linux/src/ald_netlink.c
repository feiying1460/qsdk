...
static void ald_notify(wlan_if_t vap, u_int32_t info_cmd, u_int32_t info_len, void *info_data)
{
    ...
    if (!skb) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: No memory, info_cmd = %d\n", __func__, info_cmd);
        return;
    }

    nlh = nlmsg_put(skb, pid, 0, info_cmd, info_len, 0);
    if (!nlh) {
        QDF_PRINT_INFO(vap->iv_ic->ic_print_idx, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "%s: nlmsg_put() failed, info_cmd = %d\n", __func__, info_cmd);
        kfree_skb(skb);
        return;
    }
...
#endif
{
    ...
    if ((skb = skb_dequeue(&sk->sk_receive_queue)) != NULL) {
        ...
        if (vap->ald_pid  !=  pid) {
            vap->ald_pid = pid;
            QDF_PRINT_INFO(QDF_PRINT_IDX_SHARED, QDF_MODULE_ID_ANY, QDF_TRACE_LEVEL_INFO, "[%s] Band steering events being sent to PID:%d\n",dev->name,vap->ald_pid);
        }
