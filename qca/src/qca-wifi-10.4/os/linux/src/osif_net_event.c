 ...
 * IW Events
 */
typedef uint32_t  (*__qdf_event_t)(__qdf_softc_t *sc,
				acfg_ev_data_t *data);
			...
			int type, acfg_os_event_t  *event)
{
	...
	if (unlikely(nla_put(skb, IFLA_WIRELESS,
			sizeof(acfg_os_event_t) , event) < 0))
		goto nla_put_failure;
	}

	/*
	 * TODO: If acfg is interested in ACFG_EV_PROBE_REQ then this needs to
	 * be fixed. acfg_os_event_t doesn't contain the data for
	 * ACFG_EV_PROBE_REQ
