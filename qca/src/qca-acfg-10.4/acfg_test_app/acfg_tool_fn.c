...
int wrap_add_client(char *params[])
{
    ...
    acfg_rateset_t   lrates, htrates, vhtrates;
    uint32_t status = QDF_STATUS_E_FAILURE ;
    ...
    status = acfg_add_client(vap, mac, aid, qos, lrates, htrates, vhtrates);
    return status;
}
...
int wrap_assoc_sta_info(char *params[])
{
    ...
    do {
        ...
        msg("%s %4u %3dM %4d %4d %6d %6d %4s %5s %3x %8x %6s", \
                (((pMac = addr_ntoa(psi->isi_macaddr)) == NULL)? "00:00:00:00:00:00": pMac),IEEE80211_AID(psi->isi_associd), \
                psi->isi_txrate, psi->isi_rssi, psi->isi_inact, (int32_t)psi->isi_txseqs[0], (int32_t)psi->isi_rxseqs[0], \
                getcaps(psi->isi_capinfo), (*athcap == '\0' ? "NULL": athcap), psi->isi_erp, psi->isi_state, gethtcaps(psi->isi_htcap) );

        cp += ACFG_ROUNDUP(sizeof(acfg_sta_info_t) + psi->isi_ie_len, sizeof(int32_t)) ;
...
int wrap_set_ratemask(char *params[])
{
    ...
    status = acfg_set_ratemask((uint8_t *)params[0], preamble, mask_lower32, mask_higher32);
    if (status != QDF_STATUS_SUCCESS)
...
{
    uint32_t status = QDF_STATUS_E_FAILURE ;
    uint32_t rate_flags,  nss, preamble, hw_rate;
    printf("%s[%d] params: %s %s %s %s\n", __func__,__LINE__, params[1],params[2],params[3],params[4]);

    get_hex(params[1], &rate_flags);
    get_hex(params[2], &nss);
    get_hex(params[3], &preamble);
    get_hex(params[4], &hw_rate);

    status = acfg_get_packet_power_info((uint8_t *)params[0], (uint16_t)rate_flags, (uint8_t)nss, (uint8_t)preamble, (uint8_t)hw_rate );
    return acfg_to_os_status(status) ;
}
...
int wrap_offchan_rx(char *params[])
{
    ...
    status = acfg_offchan_rx((uint8_t *)params[0], chan, scan_dur);
    if (status != QDF_STATUS_SUCCESS){
...
fntbl_t fntbl[] = {
    ...
    {"acfg_add_client", wrap_add_client, 7, wrap_add_client_params},
    ...
    {"acfg_set_ratemask", wrap_set_ratemask, 4, wrap_set_ratemask_params},
    ...
    {"acfg_get_packet_power_info", wrap_get_packet_power_info, 5, wrap_get_packet_power_info_params},
