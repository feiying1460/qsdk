        ...
        char *conffile)
{
    ...
    strcat(conffile, ".");
    strcat(conffile, (char *)vap_params->vap_name);
        ...
        uint32_t flags, uint32_t *enable_network)
{
    ...
    *enable_network = 1;
    ...
    //Set WPS related parameters
    {
        ...
        {
            if(index <= CMD_LENGTH) {
                snprintf(cmd_list[index],sizeof(cmd_list), "%s %s %s", WPA_SET_CMD_PREFIX, "config_methods",
...
acfg_set_hapd_config_params(acfg_wlan_profile_vap_params_t *vap_params)
{
            ...
            (vap_params->security_params.hs_iw_param.iw_enabled == 1))
    {
        ...
        {
            sprintf(acfg_hapd_param_list[index], "SET network_auth_type %s",
        ...
        {
            sprintf(acfg_hapd_param_list[index], "SET ipaddr_type_availability %s",
        ...
        if(vap_params->security_params.hs_iw_param.nai_realm_list[0][0] != '\0')
        {
            ...
            for(nai_realam_index = 0; nai_realam_index < vap_params->num_nai_realm_data; nai_realam_index++)
            {
                ...
                {
                    sprintf(acfg_hapd_param_list[index], "SET nai_realm %s",
        ...
        {
            sprintf(acfg_hapd_param_list[index], "SET anqp_3gpp_cell_net %s",
        ...
        {
            sprintf(acfg_hapd_param_list[index], "SET domain_name %s",
                    vap_params->security_params.hs_iw_param.domain_name_list);
            CHECK_AND_INCREAMENT(index);
        }
        sprintf(acfg_hapd_param_list[index], "SET gas_frag_limit %d",
                vap_params->security_params.hs_iw_param.gas_frag_limit);
        CHECK_AND_INCREAMENT(index);
        sprintf(acfg_hapd_param_list[index], "SET gas_comeback_delay %d",
                vap_params->security_params.hs_iw_param.gas_comeback_delay);
        CHECK_AND_INCREAMENT(index);
        if(vap_params->security_params.hs_iw_param.qos_map_set[0] != '\0')
        {
            sprintf(acfg_hapd_param_list[index], "SET qos_map_set %s",
                    vap_params->security_params.hs_iw_param.qos_map_set);
            CHECK_AND_INCREAMENT(index);
        }
        sprintf(acfg_hapd_param_list[index], "SET proxy_arp %d",
                vap_params->security_params.hs_iw_param.proxy_arp);
        CHECK_AND_INCREAMENT(index);
        sprintf(acfg_hapd_param_list[index], "SET osen %d",
                vap_params->security_params.hs_iw_param.osen);
        CHECK_AND_INCREAMENT(index);
        sprintf(acfg_hapd_param_list[index], "SET anqp_domain_id %d",
                vap_params->security_params.hs_iw_param.anqp_domain_id);
        CHECK_AND_INCREAMENT(index);
        sprintf(acfg_hapd_param_list[index], "SET hs20_deauth_req_timeout %d",
        ...
        {
            sprintf(acfg_hapd_param_list[index], "SET hs20_oper_friendly_name %s",
        ...
        {
            sprintf(acfg_hapd_param_list[index], "SET hs20_conn_capab %s",
        ...
        {
            sprintf(acfg_hapd_param_list[index], "SET subscr_remediation_url %s",
                    vap_params->security_params.hs_iw_param.subscr_remediation_url);
            CHECK_AND_INCREAMENT(index);
        }
        sprintf(acfg_hapd_param_list[index], "SET subscr_remediation_method %d",
...
acfg_hostapd_add_bss(acfg_wlan_profile_vap_params_t *vap_params, int8_t *sec)
{
    ...
    if (add_bss) {
        sprintf(buffer, "ADD %s %s %s", vap_params->vap_name, ctrl_hapd, WPA_DRIVER_ATHEROS);
